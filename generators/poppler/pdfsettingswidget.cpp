    ...
    if (m_tree && e->type() == QEvent::Paint && !m_certificatesAsked) {
        ...
        for (auto cert : certs) {
            new QTreeWidgetItem(m_tree,
                                {notAvailableIfEmpty(cert->subjectInfo(Okular::CertificateInfo::EntityInfoKey::CommonName)), notAvailableIfEmpty(cert->subjectInfo(Okular::CertificateInfo::EntityInfoKey::EmailAddress)), cert->validityEnd().toString(QStringLiteral("yyyy-MM-dd"))});
        }
        qDeleteAll(certs);
