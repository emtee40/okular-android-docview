...
void PageView::slotMouseUpAction(Okular::Action *action, Okular::FormField *form, const QPoint &globalPos)
{
    if (form && action->actionType() == Okular::Action::Script) {
        d->document->processFormMouseUpScripAction(action, form, globalPos);
    } else {
        d->document->processAction(action);
    }
}
