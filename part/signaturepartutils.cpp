...
std::optional<SigningInformation> getCertificateAndPasswordForSigning(PageView *pageView, Okular::Document *doc, SigningInformationOptions opts)
{
    ...
    if (!(opts & SigningInformationOption::BackgroundImage)) {
        ...
        QObject::connect(dialog.ui->recentBackgrounds, &QWidget::customContextMenuRequested, &dialog, [recentModel = &imagesModel, view = dialog.ui->recentBackgrounds](const QPoint &pos) {
            auto current = view->indexAt(pos);
            QAction currentImage(i18nc("Path of image", "Remove image under cursor: %1", current.data(Qt::DisplayRole).toString()));
            QAction allImages(i18n("Remove all images"));
            QAction *selected = QMenu::exec(QList<QAction *> {&currentImage, &allImages}, view->viewport()->mapToGlobal(pos), nullptr, view);
