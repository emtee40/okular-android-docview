    ...
    if (e->button() == Qt::LeftButton) {
        ...
        // Touch events may lead to the previous or next page
            ...
            case Okular::Settings::EnumSlidesTapNavigation::ForwardBackward: {
                if (e->position().x() < (geometry().width() / 2)) {
...
const void *PresentationWidget::getObjectRect(Okular::ObjectRect::ObjectType type, const QPointF& point, QRect *geometry) const
{
...
const Okular::Action *PresentationWidget::getLink(const QPointF& point, QRect *geometry) const
{
    return reinterpret_cast<const Okular::Action *>(getObjectRect(Okular::ObjectRect::Action, point, geometry));
}

const Okular::Annotation *PresentationWidget::getAnnotation(const QPointF& point, QRect *geometry) const
{
    return reinterpret_cast<const Okular::Annotation *>(getObjectRect(Okular::ObjectRect::OAnnotation, point, geometry));
}

void PresentationWidget::testCursorOnLink(const QPointF& point)
{
