...
public:
    ...
    QList<Okular::Annotation *> end() override
    {
        ...
        if (resok) {
            bool passok = false;
            const QString title = i18n("Enter password to unlock certificate %1", certToUse);
            passToUse = QInputDialog::getText(nullptr, i18n("Enter certificate password (if any)"), title, QLineEdit::Password, QString(), &passok);
    ...
    bool isAccepted() const { return !certToUse.isEmpty(); }
