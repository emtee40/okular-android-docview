...
public:
    ...
    void stateChanged(int, QAudio::State);
...
//    format.setSampleType(QAudioFormat::UnSignedInt);
    ...
    case Sound::Embedded: {
        ...
        if (!filedata.isEmpty()) {
            ...
            int newid = newId();
            QObject::connect(data->m_output, &QAudioOutput::stateChanged, q, [this, newid](QAudio::State state) { stateChanged(newid, state); });
...
void AudioPlayerPrivate::dataDownloaded(KJob *job, QByteArray &data)
{
    ...
    playData->play();
    int newid = newId();
    QObject::connect(playData->m_output, &QAudioOutput::stateChanged, q, [this, newid](QAudio::State state) { stateChanged(newid, state); });
    m_playing.insert(newid, playData);
    m_state = AudioPlayer::PlayingState;
}

void AudioPlayerPrivate::stateChanged(int id, QAudio::State state)
{
